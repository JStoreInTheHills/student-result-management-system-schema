...
const init = () => {
  $.ajax({
    url: "../queries/view_teachers/get_teachers_details.php",
    type: "GET",
    data: formData,
  }).done((response) => {
    ...
    arr.forEach((item) => {
      heading.html(
        `<span><img class="img-profile rounded-circle" width="50"  src="/src/img/download.png"></span> Welcome, ${item.first_name} ${item.second_name} ${item.last_name}.`
      );
      teachers_name.html(
        `<span class="text-gray-600">Name</span> : ${item.first_name} ${item.second_name} ${item.last_name}`
      );
      id_no.html(
        `<span class="text-gray-600">ID Number </span> : ${item.id_no}`
      );
      email.html(
        `<span class="text-gray-600" >Email Address  </span> : <a href=""> ${item.email_address}</a>`
      );
      phone.html(
        `<span class="text-gray-600">Phone Number </span>: ${item.phone_number}`
      );
      CreationDate.html(
        `<span class="text-gray-600">Date of Registration </span>: ${item.created_at}`
      );
      title.html(`${item.first_name} ${item.second_name} ${item.last_name} || Teachers`);
      edit_teacher_btn.html(
        `<span><i class="fas fa-edit"></i> </span> Edit ${item.first_name}`
      );

      checkTeachersStatus(item);

      $("#teacher_address").html(
        `<span class="text-gray-800">Phone Number </span> : 0${item.phone_number}`
      );
      ...
      $("#edit_teacher_heading").html(`${item.first_name} ${item.second_name} ${item.last_name}`);

      $("#edit_teachers_name").val(`${item.first_name} ${item.second_name} ${item.last_name}`);
      $("#edit_id_no").val(item.id_no);
      $("#edit_phone").val(item.phone_number);
